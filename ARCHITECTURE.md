...
| Bedrock + AgentCore Runtime | Typed request interpretation, tool sequencing through the gateway and explanation | Deterministic checks inside the tools establish eligibility. Proposals that mention reservations or connections use a source-based explanation, so the model cannot imply a booking or calculate a different connection shortfall. No-match, unsupported-trip and booking outcomes also use text built directly from the checked result. Other proposal prose remains model output. |
...
7. `select_itinerary`: a fresh Aurora query rechecks prices and availability; hard constraints apply before ranking. The runtime emits the plan, then checks the complete explanation before displaying it. A proposal cannot claim a reservation; a no-match answer uses the checked limits and minimum price. The session manager stores the conversation in Memory.
8. On one confirmed click, a booking turn calls `book_trip`. The gateway's Cedar policies permit or forbid the call; if permitted, the Lambda reserves one seat and one room in one atomic statement and returns a reference. Both inventory rows are locked first; only a newly inserted booking consumes stock. A unique booking key per session makes retries safe.
...
`backend/tools.py` implements the six tools in the Lambda over `backend/services.py` (bounded SQL, openCypher and SDK calls) and `backend/planner.py` (deterministic feasibility and ordering). `backend/main.py` owns the typed contract, the Strands agent, the gateway client, the trace hooks and the event stream; `backend/gateway_auth.py` signs gateway requests with SigV4. Before each call, the runtime replaces model-supplied arguments with values from the validated contract and selected itinerary, including the actual confirmation flag. Unsupported trips stop the sequence, and a planning turn cannot book. Neither the model nor the browser can submit arbitrary SQL: the model sees six tool schemas, and the gateway’s policy engine decides whether each call may run.

## Streaming and observability

The SSE stream contains run lifecycle (with the OpenTelemetry trace id), 24 semantic events (four per component) and a separate gateway discovery event, a validated plan, checked answer text, model usage, booking outcomes and completion/error events. A supported planning run produces all 24 semantic events; unsupported requests stop after their limitation is established. Each semantic event has an ID, timestamp and explicit service responsibility; completed tool calls carry real request IDs and measured elapsed time. The semantic path streams tool events immediately, then sends the complete checked answer in one text event. Model-only comparison mode streams the model's text directly.
...
The fictional actor is `alex-onward`, with long-term namespace `/onward/actors/{actorId}/preferences/`. New conversations get new runtime sessions. The typed trip contract and latest selection are also saved as application state in a separate AgentCore short-term session, so a runtime restart does not reset a follow-up’s constraints. Restored model context contains user-facing text; orphaned tool and signed reasoning blocks are excluded. Both SDK message records and plain preference conversations can be restored. Saved evidence is retained.

Ordinary conversation and state events use extractionMode SKIP; an explicit remember request uses long-term extraction. New conversations retain actor preferences while resetting the conversation contract. Memory off removes the ranking contribution; it does not delete past events. Asking to remember a preference stores conversation evidence immediately; asynchronous extraction may become available later.
