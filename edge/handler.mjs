...
export const handler=awslambda.streamifyResponse(async(event,responseStream)=>{
  ...
  try{
    ...
    if(path==='/api/scenario'){
      ...
      const result=await rds.send(new ExecuteStatementCommand({resourceArn:cfg.clusterArn,secretArn:READER_SECRET||cfg.secretArn,
      ...
      cachedHealth=null;
      return send(responseStream,200,{offer:JSON.parse(result.formattedRecords)[0],requestId:result.$metadata.requestId});
    }
