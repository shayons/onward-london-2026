...
const serviceRoles=[{bedrock:'Turns the request into clear trip requirements',agentcore:'Runs the search steps'},{aurora:'Connects names to the same people and places'},{memory:'Remembered traveller preferences',bedrock:'Turns hotel descriptions into comparable numbers',aurora:'Combines word matches and meaning matches'},{aurora:'SQL: fare + bag + hotel + transfer'},{neptune:'Connected journey paths',s3:'Versioned timing and connection policies'},{aurora:'Checks the trip against current prices and stock'},{agentcore:'Gateway policy check (Cedar) before the tool runs',aurora:'One seat and one room reserved atomically'}];
...
export const layerContributions=[
 ...
 'Give “tomorrow” a date and “nearby” a walking limit. Search hotel descriptions by words and by meaning, then combine the results. Preferences help choose between trips that already meet the requirements.',
 ...
 'Use the stored trip checklist and check prices and availability again before recommending a trip.',
 'Check Alex’s confirmation and the booking rules. Only an allowed booking can reserve a seat and a room together. These are fictional reservations; no supplier is contacted.'
];

export function hotelTiles(hotels,live=false,maxWalkMinutes=20){return `<div class="hotel-grid ${live?'live-hotel-grid':''}">${hotels.map((h,i)=>{const walk=h.walk_minutes??h.walkMinutes,price=h.night_pence??h.nightPence;const score=value=>live&&Number.isFinite(Number(value))?Number(value).toFixed(2):'—';return `<article class="hotel-tile"><img src="${hotelImages[h.id]||''}" alt="AI-generated illustration for fictional ${esc(h.name)}" loading="lazy"><div class="hotel-tile-body"><h3>${esc(h.name)}</h3><p>${esc(h.description)}</p><div class="hotel-stats"><span>Walk <strong>${walk}m</strong></span><span><strong>${money(price)}</strong> / night</span><span title="Keyword relevance score; not a probability">Word match <strong>${score(h.lexical_score)}</strong></span><span title="Vector similarity score; not a probability">Meaning match <strong>${score(h.similarity)}</strong></span></div><div class="hotel-ranking">${live?`<span>Combined rank <strong>${i+1}</strong></span>`:'<span>Scores appear after a live search</span>'}<span class="${walk>maxWalkMinutes?'outside-walk':'within-walk'}">${walk>maxWalkMinutes?`Over the ${maxWalkMinutes}-minute walk limit`:`Within the ${maxWalkMinutes}-minute walk limit`}</span></div><small>Fictional hotel · AI-generated illustration</small></div></article>`;}).join('')}</div>`;}

export function preparation(events=[]){
 const search=events.findLast(e=>e.type==='trace'&&e.layer===2&&e.phase==='result')?.evidence;
 const matches=search?.hotels;
 const returned=events.findLast(e=>e.type==='trace'&&e.layer===4&&e.phase==='result')?.evidence?.sources||[];
 return `<header class="prepare-header"><button class="quiet-button" data-action="back-concierge">← Back to the concierge</button><h1 id="prepare-title" tabindex="-1">Data preparation</h1><p>Before Alex asks, we connect names, make costs consistent, prepare hotel descriptions for search and link each journey to its rules.</p><span class="seed-note">Fictional travel data · 15 September 2026 · searches run on AWS</span></header>
 <div class="prepare-content">
 <section class="preparation-overview"><div><h2>One meeting. Several things to check.</h2><p>Alex must reach Ribeira Design Studio by 14:00, with one nearby hotel night and the complete outbound trip strictly under £650.</p><p>A hotel can sound right and still be too far away. A cheap flight can still miss the meeting. We need to check the whole trip.</p></div><figure><img src="assets/concierge/venue-ribeira.jpg" alt="AI-generated illustration of the fictional Ribeira Design Studio"><figcaption>Fictional meeting venue · AI-generated illustration</figcaption></figure></section>
 <section><h2>Make the source records consistent</h2><p>Use the same airport codes everywhere. Store money in pence so the totals stay exact. Show the bag charge separately.</p>${serviceBadges(['aurora'])}<div class="table-scroll"><table><thead><tr><th>Flight</th><th>Fare only</th><th>Fare pence</th><th>Bag pence</th><th>Seed seats</th></tr></thead><tbody>${data.offers.map(o=>`<tr><th>${o.id}</th><td>${money(o.farePence)}</td><td><code>${o.farePence}</code></td><td><code>${o.bagPence}</code></td><td>${o.seats}</td></tr>`).join('')}</tbody></table></div><p class="seed-note">This table documents the seed snapshot and lists the flight component only; the concierge quotes complete prices. The AX218 inventory control changes the Aurora record; each live run reads its current availability.</p></section>
 <section><h2>A complete price needs a definition.</h2><div class="formula"><span>Outbound fare</span><b>+</b><span>Checked bags</span><b>+</b><span>Hotel with taxes</span><b>+</b><span>Airport transfer</span></div><p>“Under £650” means the complete price must be below £650. A trip costing exactly £650 does not fit. Travel to Heathrow and return travel are outside this trip's scope.</p>${(()=>{const ax=data.offers.find(o=>o.id==='AX218'),patio=data.hotels.find(h=>h.id==='patio-house'),forum=data.hotels.find(h=>h.id==='forum-rooms'),transfer=data.transfer.pricePence;if(!ax||!patio||!forum)return '';const total=h=>ax.farePence+ax.bagPence+h.nightPence+transfer;return `<p class="seed-note">Worked example: AX218 with Pátio House is ${money(ax.farePence)} fare + ${money(ax.bagPence)} bag + ${money(patio.nightPence)} hotel + ${money(transfer)} transfer = <strong>${money(total(patio))}</strong>. With Forum Rooms at ${money(forum.nightPence)} a night the same flight comes to <strong>${money(total(forum))}</strong>, which is why a no-fit under £450 reports £470 as the minimum feasible price.</p>`;})()}</section>
 <section id="hotel-preparation"><h2>Find hotels by words and by meaning</h2><p>Keyword search finds the words in a description. Vector search also finds similar meaning: “somewhere quiet” can match “courtyard-facing rooms on a residential lane”. Hybrid search combines both rankings. The walking limit is checked separately.</p>${serviceBadges(['bedrock','aurora'])}${hotelTiles(matches||data.hotels,Boolean(matches),search?.maxWalkMinutes??search?.request?.maxWalkMinutes??20)}<p class="seed-note">${matches?'Scores from the latest search shown in this conversation. Word and meaning scores measure different things; we combine their ranking positions, not their raw scores. They are not probabilities.':'Seed descriptions, prices and walking times. Run the question and reveal Disambiguation to see the actual retrieval scores here.'}</p></section>
 <section><h2>Relationships make the whole journey visible.</h2><p>The graph links each flight, airport, transfer and hotel to the same meeting. The application then checks that the steps connect and leave enough time.</p>${serviceBadges(['neptune','s3'])}<div class="journey-diagram" role="img" aria-label="A flight reaches Lisbon airport. Add 45 minutes for arrivals and 35 minutes for the transfer to reach the meeting. Hotels connect to the meeting by walking edges."><div><span>Flight legs</span><b>→</b><span>Lisbon airport</span><b>→</b><span>45 min arrivals</span><b>→</b><span>35 min transfer</span><b>→</b><strong>Meeting venue</strong></div><div><span>Hotel</span><b>→</b><span>Walk of up to 20 min</span><b>→</b><strong>Same venue ID</strong></div></div><div class="route-lessons"><article><h3>The late arrival</h3><p>AX404 lands at <strong>13:20</strong>. Add the allowance and transfer: Alex reaches the venue at <strong>14:40</strong>. A flight arriving before the meeting still fails.</p></article><article><h3>The short connection</h3><p>ME615 arrives Madrid at <strong>10:00 +02:00</strong> and departs at <strong>10:45 +02:00</strong>. The <strong>45 elapsed minutes</strong> fail the <strong>60-minute</strong> rule.</p></article></div></section>
 <section><h2>Remember what Alex usually prefers.</h2>${serviceBadges(['memory'])}<blockquote>${esc(data.memory.quote)}</blockquote><p>AgentCore Memory stores Alex’s earlier conversation and prepares preferences for later searches. This preparation takes time. Each search shows whether it read an extracted preference or the original conversation. An allergy is kept separately as an explicit declaration; it is never guessed from a preference.</p><p>Alex can say “window for this trip” or ask for a livelier hotel. The current request wins, and a one-trip change does not rewrite the usual preference. Memory helps with taste; the source records still provide prices, schedules and availability.</p></section>
 ...
 <section><h2>Choose the search for the question.</h2><p>Start with what you need to know. You can combine these approaches, and several can run in the same database.</p><div class="retrieval-choices">${[
 ['aurora','Exact facts / SQL','What is the price, and is it available?','You need exact amounts, joins or clear limits.','Names and wording vary, so an exact match misses a useful record.','Aurora adds the flight, bag, hotel and transfer, and checks seats and rooms.'],
 ['aurora','Keyword search','Which records contain these words?','Flight codes, names and distinctive terms matter.','Alex says “quiet”, but the hotel description uses different words.','PostgreSQL full-text search ranks the hotel descriptions by word matches.'],
 ['bedrock','Embeddings','How can we compare meaning?','You need numbers that place similar descriptions close together.','The numbers alone do not retrieve a record or prove a current fact.','Bedrock Titan turns hotel descriptions and the search wording into vectors: lists of numbers.'],
 ['aurora','Vector search','Which descriptions have similar meaning?','Different wording can describe the same need.','A close meaning match may still be too far away, too expensive or unavailable.','Aurora pgvector compares the search vector with the stored hotel vectors.'],
 ['aurora','Hybrid search','Which results match the words or the meaning?','Exact terms and different ways of saying the same thing both matter.','Combining rankings alone does not check the budget or the journey.','Aurora combines word and vector ranking positions. This is called reciprocal rank fusion.'],
 ['neptune','Knowledge graph','How do the parts of the trip connect?','You need to follow several connected steps or explore changing relationships.','A connection on a map does not prove there is enough time to use it.','Neptune returns the flight, transfer and walking links. Application rules check the times.']
].map(([key,name,answers,wins,fails,onward])=>`<article><div class="choice-name"><img src="assets/aws/${key}.svg" alt="" width="34" height="34"><div><h3>${name}</h3><strong>${serviceNames[key]}</strong></div></div><dl><dt>Answers</dt><dd>${answers}</dd><dt>Wins when</dt><dd>${wins}</dd><dt>Fails when</dt><dd>${fails}</dd><dt>In Onward</dt><dd class="choice-onward">${onward}</dd></dl></article>`).join('')}</div><p class="choice-order"><strong>Match meaning. Check facts. Follow connections.</strong> Onward finds hotel matches, prices complete trips, checks the journeys, then refreshes prices and availability before choosing. The result must meet every trip requirement.</p><p class="seed-note">Start with a relational database when your joins are simple. Add vector or hybrid search when wording varies. Consider a graph when following relationships becomes a central part of the work. These are six responsibilities, not a requirement to buy six databases.</p></section>
...
export function talkNotes(){return `<p class="dialog-lead">Ten minutes: a familiar problem, the data behind it, and a practical choice of search.</p><ol class="talk-track">${[
 ['0:00–1:00','Meet Alex','Her flight is cancelled, but her 14:00 meeting stays. The whole replacement trip must cost under £650. People and travel offers are fictional; the AWS calls are real.'],
 ['1:00–2:00','Show the preparation','Open Data preparation. We give places shared names, count all costs, prepare hotel descriptions for meaning search and connect each journey to its rules. Return to Concierge.'],
 ['2:00–4:00','Ask once, show what it means','Select Presenter, reveal the cancellation and send the request. Use Next layer three times. Explain the goal, shared names and the meaning of “nearby”. Keyword search matches words; vector search matches similar meaning; hybrid combines them.'],
 ['4:00–6:00','Check the whole trip','Advance Metrics and Relationships. A £255 fare becomes a £490 complete trip. AX404 reaches the meeting at 14:40. ME615 leaves only 45 minutes to connect against a 60-minute rule. The graph supplies links; the code checks time.'],
 ['6:00–7:00','Show the answer','Advance Verified examples. The usual quiet-hotel result is AX218 + Pátio House, £490, meeting at 12:25. Read the actual result on screen: preferences and availability can change it. Nothing is reserved.'],
 ['7:00–8:00','Change one thing','Select Auto reveal, then Somewhere livelier. The hotel should change while the same budget and deadline still apply. Use the £450 question instead if you want to show an honest no-match.'],
 ['8:00–9:00','Choose the approach','Open Architecture. Exact facts need database checks. Different wording benefits from vectors. Mixed needs favour hybrid search. Connected journeys may benefit from a graph. Several roles can share one database.'],
 ['9:00–10:00','Land the point and leave room','The data makes the useful answer possible: shared names, complete costs, searchable descriptions, connected steps and source-backed rules. Use the remaining time for a question or a slow service.']
 ].map(([time,title,body])=>`<li><time>${time}</time><h3>${title}</h3><p>${body}</p></li>`).join('')}</ol><p class="seed-note">Booking is an optional extension. One click on Book this for Alex checks the booking rules and reserves fictional stock. Use a fresh conversation for the refusal example. Pause holds the display; AWS keeps running. Replay shows the recorded run.</p>`;}
...
function memoryShowcase(){
 ...
 <div class="memory-integration">
  <div class="memory-integration-heading"><span class="memory-eyebrow">How Memory integrates with Aurora</span><h3>Separate stores, connected by the search tool</h3><p><strong>Aurora is not the backing store for AgentCore Memory.</strong> Conversations and preferences stay in AgentCore Memory. Aurora stores the trip records and current inventory.</p></div>
  <ol>
   ...
   <li><span>2</span><div><strong>The search tool applies the current request</strong><p>The current request overrides remembered preference. Aurora's traveller entity supplies explicit declarations, such as the shellfish allergy, as separate authoritative application data.</p></div></li>
   <li><span>3</span><div><strong>Aurora searches and validates products</strong><p>The search tool maps the preference to the prepared vocabulary, Bedrock creates the query embedding, and Aurora runs pgvector plus full-text rank fusion. Aurora SQL then prices every flight-and-hotel bundle; the application rules discard the ones that fail, and the price query runs again before ranking.</p></div></li>
  </ol>
  <p class="memory-integration-result"><span>AgentCore Memory</span><b>personal context</b><i>→ Search tool →</i><span>Amazon Bedrock</span><b>query embedding</b><i>→</i><span>Aurora PostgreSQL</span><b>profile, search, price, stock</b></p>
 </div>
...
export function briefing(health){
 ...
 <p>Onward uses several engines on purpose, and each handles a different part of the demonstration. Several roles could share one database in a smaller application. Relational truth, keyword matching, vector similarity, rank fusion and the booking ledger all live in Aurora; reachability lives in Neptune; provenance lives in S3; the traveller's history lives in AgentCore Memory; the rules about acting live in the gateway's policy engine.</p>
 ...
 <ul class="briefing-list">
  <li><strong>Search steps</strong> accumulate as the run proceeds — each semantic responsibility with its own one-line summary and the AWS marks of the services doing that step — then collapse to a single line when the run completes.</li>
