...
import {hotelImages,hotelTiles,serviceBadges,serviceNames,layerServices,layerContributions,preparation,briefing,talkNotes} from './presentation.js?v=review-ui-20260911c';
...
const layers=[
  {name:'Business context',plain:'Set the goal',phrase:'“make my meeting”',question:'What does success mean?',service:'Bedrock intent · AgentCore orchestration'},
  {name:'Ontology',plain:'Connect names and places',phrase:'“me / Heathrow / my Lisbon meeting”',question:'Which people, places and things?',service:'Aurora · shared entity IDs and aliases'},
  {name:'Disambiguation',plain:'Understand the words',phrase:'“tomorrow / nearby / quiet”',question:'What do these words mean here?',service:'Aurora hybrid retrieval · AgentCore Memory'},
  {name:'Metrics',plain:'Count every cost',phrase:'“the whole trip under £650”',question:'What is the complete price?',service:'Aurora · complete-price SQL'},
  {name:'Relationships',plain:'Check the whole journey',phrase:'“will this journey work?”',question:'Will the whole journey work?',service:'Neptune journey paths · S3 source policies'},
  {name:'Verified examples',plain:'Check the answer',phrase:'“which supported trip should I choose?”',question:'Is the answer supported?',service:'Aurora · query patterns and current facts'},
  {name:'Booking',plain:'Check permission to book',phrase:'“book it for me”',question:'Is the action allowed?',service:'AgentCore Gateway policy · Aurora inventory'}
];
const SEMANTIC=6;
...
const state={generation:0,changingStock:false,stopping:false,selectionTurn:null,bookingAttempted:false,foldTimer:null,sessionId:'onward-'+crypto.randomUUID(),messages:[],health:null,active:null,cancelled:false,presenter:false,modelId:null,
  paused:false,pace:450,timer:null,revealTimer:null,revealDeadline:null,openLayer:null,follow:true,traceOpen:true,
  soldOut:false,semantic:true,inspectedTurn:null,replaying:false,queue:[],cursor:0,visible:[],pendingLayer:null,turn:0,tripExpanded:true,animatedEvents:new Set(),dialogFocus:null};
...
function intro(){return `<section class="story-intro${state.cancelled?' challenge':''}"><h2>${state.cancelled?'The flight changed. The meeting hasn’t.':'A meeting to make in Lisbon.'}</h2><p>${state.cancelled?'Find a new journey that meets all three limits. The flight, bag, hotel and transfer must fit together.':'Alex flies tomorrow for a client meeting. Reveal the cancellation to start the search for another way there.'}</p><dl class="trip-limits" aria-label="Trip requirements"><div><dt>At the meeting</dt><dd>By 14:00</dd><small>Lisbon time</small></div><div><dt>Whole trip</dt><dd>Under £650</dd><small>Flight + bag + hotel + transfer</small></div><div><dt>Hotel nearby</dt><dd>20 min walk</dd><small>One night, 15–16 September</small></div></dl></section>`;}
function renderTrip(){
...
function goalSummary(message){
 const request=message.request||message.plan?.request;if(!request||message.modelOnly)return '';
 return `<dl class="goal-summary" aria-label="Requirements for this request"><div><dt>Meeting</dt><dd>By ${esc(request.deadline)}</dd></div><div><dt>Complete trip</dt><dd>Under ${money(request.budgetPence)}</dd></div>${request.maxWalkMinutes!==undefined?`<div><dt>Hotel walk</dt><dd>Up to ${esc(request.maxWalkMinutes)} min</dd></div>`:''}</dl>`;
}
...
function refusalCard(m){
 const denied=m.policyDecision==='deny';
 return `<section class="booking-refused"><h3>${denied?'Booking not allowed.':'Booking not confirmed.'}</h3><p>${denied?esc(m.bookingRefused):'An error interrupted the booking check. The reservation may have completed.'}</p><p>${denied?'Nothing was reserved. The booking rules blocked this request before it could change the inventory.':'Retry this booking to check for the saved reservation before choosing another trip.'}</p>${denied?'':`<button class="quiet-button" data-action="retry" data-retry-turn="${m.turn}">Check this booking again ${icon('arrow')}</button>`}</section>`;
}

const FOLD_MS=720;
function retrievalPreview(m){
 if(!m.hotelResults)return '';
 const body=`${personalContext(m.travellerContext)}<h3>Hotel matches: words, meaning and walking time</h3>${hotelTiles(m.hotelResults,true,m.travellerRequest?.maxWalkMinutes??20)}`;
 if(m.hotelFold==='folding')return `<section class="retrieval-preview folding" data-fold-start="${m.hotelFoldStart}" aria-hidden="true"><div>${body}</div></section>`;
 if(m.plan){const chosen=m.plan.selected?.hotel?.name;return `<details class="retrieval-folded"><summary>${icon('chevron')} Search compared ${m.hotelResults.length} hotels${chosen?` · ${esc(chosen)} selected`:' · none eligible'}</summary><section class="retrieval-preview">${body}</section></details>`;}
...
function reasoning(m){
 if(m.modelOnly){const body=`<ol class="reasoning"><li class="step ${m.pending?'active':'done'}"><span class="step-mark">${m.pending?'<span class="step-indicator"></span>':icon('check')}</span><div><strong>Model only</strong><p>The semantic layer is off: no entity registry, no retrieval, no prices, no journey graph, no memory and no policy. Whatever follows is the model’s own.</p></div></li></ol>`;return m.pending?body:`<details class="reasoning-done"><summary>Model only · 0 of ${SEMANTIC} semantic responsibilities resolved ${icon('chevron')}</summary>${body}</details>`;}
 const steps=m.steps||[];
 const opening=m.pending&&!steps.length?`<li class="step active"><span class="step-mark"><span class="step-indicator"></span></span><div><strong>Reading the request</strong><p>${esc(m.status||'Connecting to the live agent…')}</p></div></li>`:'';
 const rows=steps.map(step=>`<li class="step ${step.error?'refused':step.done?'done':'active'}"><span class="step-mark">${step.error?icon('close'):step.done?icon('check'):'<span class="step-indicator"></span>'}</span><div><strong>${esc(layers[step.layer].plain)}</strong><p>${esc(step.summary||'')}</p><span class="step-marks">${serviceMarks(layerServices[step.layer])}</span></div></li>`).join('');
 if(!opening&&!rows)return '';
 const body=`<ol class="reasoning">${opening}${rows}</ol>`;
 if(m.pending&&!m.text)return body;
...
function traceMessage(){return state.inspectedTurn===null?currentAssistant():state.messages.find(m=>m.role==='assistant'&&m.turn===state.inspectedTurn);}
function isBookingTrace(){
 const message=traceMessage();
 return !message?.modelOnly&&Boolean(message?.bookingTurn||message?.request?.action==='book'||traceEvents().some(e=>e.type==='booking'||e.type==='trace'&&e.layer===SEMANTIC));
}
function streamAnswer(message){
 ...
 node.innerHTML=answerProse(message);node.classList.toggle('streaming',message.pending);
 if(nearBottom)view.scrollTop=view.scrollHeight;
 return true;
}
function answerProse(message){
 let text=message.shownText??message.text;
 // Keep a received, partly revealed bold phrase formatted while its final words appear.
 if(message.pending&&(text.match(/\*\*/g)||[]).length%2)text+='**';
 return prose(text);
}
function answerMarkup(message){
 if(!message.text&&!message.shownText)return '';
 return `<div class="answer-text${message.pending?' streaming':''}">${answerProse(message)}</div>`;
}
function completeDisplay(message){
 if(!message.streamDone||message.shownText!==message.text)return;
 message.pending=false;renderChat();renderControls();
}
function revealAnswer(message,complete=false){
 if(state.answerReveal?.message!==message){
  cancelAnswerReveal();
  const generation=state.generation;
  const valid=()=>generation===state.generation&&message===currentAssistant();
  const control=createAnswerReveal({
   requestFrame:callback=>requestAnimationFrame(callback),cancelFrame:id=>cancelAnimationFrame(id),
   reducedMotion:()=>matchMedia('(prefers-reduced-motion: reduce)').matches,
   onUpdate:text=>{if(!valid())return;message.shownText=text;if(!streamAnswer(message))renderChat();},
   onIdle:()=>{if(valid())completeDisplay(message);}
  });
  state.answerReveal={message,control};
 }
 state.answerReveal.control.setText(message.text,{complete});
}
function cancelAnswerReveal(){state.answerReveal?.control.cancel();state.answerReveal=null;}
function renderControls(){
 const pending=state.queue.length-state.cursor,traces=traceEvents().filter(e=>e.type==='trace'),complete=new Set(traces.filter(e=>e.phase==='result'&&!e.error&&e.layer<SEMANTIC).map(e=>e.layer)).size;
 const textPending=Boolean(state.answerReveal?.control.pending&&currentAssistant()?.pending);
 const receiving=Boolean(state.active),canReveal=pending>0||receiving||textPending;
 ...
 const mode=currentAssistant()?.modelOnly&&state.inspectedTurn===null?'Model only · the semantic layer is off':state.inspectedTurn!==null?`Earlier turn ${state.inspectedTurn} · recorded evidence`:state.replaying?'Recorded run · no new AWS calls':receiving?(state.paused?'Display paused · agent continues':'Receiving live AWS events'):pending||textPending?(state.paused?'Display paused · run received':textPending?'Showing the received answer':'Revealing received evidence'):failed?'Run interrupted · evidence retained':finished?'Run complete · evidence available':'Ready for your question';
 const controls=[['play-pause',state.paused?'Resume':'Pause reveal',state.paused?'play':'pause',canReveal],['next-event','Next event','step',canReveal],['next-layer','Next layer','arrow',canStepLayer],['replay','Replay','replay',state.queue.some(e=>e.type==='done')&&!receiving&&!currentAssistant()?.pending]];
 const controlsHtml=controls.map(([action,label,glyph,enabled])=>`<button class="dock-control${action==='next-layer'&&state.presenter?' primary-step':''}" data-action="${action}" ${enabled?'':'disabled'} title="${label}">${icon(glyph)}<span>${label}</span></button>`).join('');
 if(state.controlsHtml!==controlsHtml){$('trace-controls').innerHTML=controlsHtml;state.controlsHtml=controlsHtml;}
 if(isBookingTrace()){
  const message=traceMessage(),booking=traceEvents().findLast(e=>e.type==='booking');
  const status=booking?.booking?'Booking confirmed':booking?.policyDecision==='deny'?'Booking refused':booking||message?.failed?'Booking not confirmed':'Booking result not yet shown';
  const proposal=state.messages.findLast(m=>m.role==='assistant'&&m.plan?.selected&&m.turn<message?.turn);
  $('trace-progress').innerHTML=`<span>${status}</span><small>A separate check after the trip search.</small>${proposal?`<button class="quiet-button" data-source="0" data-source-turn="${proposal.turn}">View the earlier trip checks ${icon('arrow')}</button>`:''}${pending&&state.paused?'<small>Use Next layer to continue.</small>':''}`;
 }else{
  $('trace-progress').innerHTML=`<div class="check-progress" role="progressbar" aria-label="Trip steps shown" aria-valuemin="0" aria-valuemax="${SEMANTIC}" aria-valuenow="${complete}">${layers.slice(0,SEMANTIC).map((_,i)=>`<span class="${traces.some(e=>e.layer===i&&e.phase==='result'&&!e.error)?'checked':''}"></span>`).join('')}</div><span>${traces.length?`${complete} of ${SEMANTIC} steps shown`:'Prepared data supports every check.'}</span>${pending&&state.paused?'<small>Use Next layer to continue.</small>':''}`;
 }
...
function layerResolution(layer,events){
  ...
  const request=evidence.request||traceEvents().findLast(e=>e.type==='trace'&&e.evidence?.request)?.evidence.request||{};
  if(!result)return {value:'Waiting for a result from this run.',detail:layers[layer].question};
  if(layer===0){
    return {
      ...
      detail:'Alex needs to reach the meeting, including the journey from the airport.'
    };
  }
  if(layer===1){
    ...
    const destination=request.destination||'Unknown destination';
    const venue=entities.find(e=>e.kind==='venue')?.name||'the meeting venue';
    return {
      value:`${traveller} · ${origin} → ${destination} · ${venue}`,
      detail:'Shared names and IDs connect the flight, transfer, hotel and meeting records.'
    };
  }
  if(layer===2){
    ...
    return {
      value:`${evidence.date||request.travelDate} · up to ${evidence.maxWalkMinutes??request.maxWalkMinutes} min walk · ${evidence.preferenceApplied?'hotel preferences applied':'no hotel preference applied'}`,
      detail:`${memory} ${search} A close match must still meet the trip requirements.`
    };
  }
  if(layer===3){
    const budget=request.budgetPence;
    return {
      value:`Fare + ${request.bags} checked ${request.bags===1?'bag':'bags'} + hotel with taxes + airport transfer: under ${money(budget)}.`,
      detail:'Every cost counts. The total must be below the budget, not equal to it. Travel to Heathrow and return travel are excluded.'
    };
  }
  if(layer===4){
    return {
  ...
  if(layer===SEMANTIC){
    const refused=result.evidence?.policyDecision==='deny';
    return {
      value:result?.summary||'Waiting for the gateway’s policy decision.',
      detail:refused?'The booking rules blocked this request before the booking tool ran. Nothing was reserved.':result.error?'The booking did not finish. See the returned evidence for its status.':'The booking rules checked Alex’s confirmation and the trip details. A successful demo booking reserves a seat and room together.'
    };
  }
  return {
...
function updateTraceHeading(){const governed=isBookingTrace();const h=document.querySelector('.trace-heading h1'),p=document.querySelector('.trace-heading p');if(!h)return;h.textContent=governed?'From an answer to a booking':'How the data helps';p.textContent=governed?'Confirm the request, then check permission to book.':'Six steps from a question to a workable trip.';}
function renderTraces(){updateTraceHeading();
 const shown=traceEvents(),governed=isBookingTrace(),scroller=$('trace-scroll'),scroll=scroller.scrollTop;
...
function apply(e){
  ...
  if(e.type==='booking'){
    ...
    if(e.booking){
      announce(state.replaying?`Recorded booking confirmation. Reference ${e.booking.reference}; no new reservation was made.`:`Demo booking confirmed. Reference ${e.booking.reference}: a seat on ${e.booking.offerId} and a room are reserved.`);
      if(!state.replaying&&e.booking.offerId==='AX218'&&Number.isSafeInteger(e.booking.seatsLeft))updateStock(e.booking.seatsLeft);
    }
    message.booking=e.booking||null;message.bookingRefused=e.booking?null:(e.refused||'The gateway did not complete the booking.');message.policyDecision=e.policyDecision||null;state.chatAnchor='.assistant-message:last-child .booking-card, .assistant-message:last-child .booking-refused';
  }
  if(e.type==='trace'){
    message.status=e.summary;message.currentLayer=e.layer;
    if(e.layer===0&&e.phase==='result'&&e.evidence?.request)message.request=e.evidence.request;
    ...
    else message.steps.push({layer:e.layer,summary:e.summary,done:e.phase==='result',error:e.error===true});if(e.layer===2&&e.phase==='result'){message.hotelResults=e.evidence?.hotels;message.travellerContext=e.evidence?.travellerContext;message.travellerRequest=e.evidence?.request;state.chatAnchor='.assistant-message:last-child .retrieval-preview';}
    if(state.follow)state.openLayer=e.layer;
    renderTraces();
  }
  if(e.type==='plan'){state.selectionTurn=e.plan?.selected?message.turn:null;const landing='.assistant-message:last-child .itinerary-card, .assistant-message:last-child .no-match';if(message.hotelResults){message.hotelFold='folding';message.hotelFoldStart=Date.now();state.holdReveal=FOLD_MS+60;state.chatAnchor='.assistant-message:last-child .retrieval-preview';const generation=state.generation;state.foldTimer=setTimeout(()=>{if(generation!==state.generation||message!==currentAssistant()||message.hotelFold!=='folding')return;state.foldTimer=null;message.hotelFold='folded';state.chatAnchor=landing;renderChat();},FOLD_MS+40);}else state.chatAnchor=landing;message.documents=state.visible.findLast(v=>v.type==='trace'&&v.layer===4&&v.phase==='result')?.evidence?.sources||[];message.plan=e.plan;}
  if(e.type==='token'){message.text+=e.text;revealAnswer(message);return;}
  if(e.type==='answer'){message.text=e.text;message.kind=e.kind;message.memoryEventId=e.memoryEventId;revealAnswer(message,true);}
  if(e.type==='done'){state.pendingLayer=null;message.streamDone=true;if(message.text)revealAnswer(message,true);message.pending=(message.shownText??'')!==message.text;message.elapsedMs=e.elapsedMs;message.savedEvents=[...state.visible];}
  if(e.type==='error'){cancelAnswerReveal();state.pendingLayer=null;state.selectionTurn=null;message.pending=false;message.failed=e.message;message.savedEvents=[...state.visible];state.openLayer=e.layer??state.openLayer;renderTraces();}
  if(e.type!=='usage'&&e.type!=='run')renderChat();
}
function stopPlayback(){clearTimeout(state.timer);state.timer=null;}
function cancelFold(){clearTimeout(state.foldTimer);state.foldTimer=null;state.holdReveal=0;const message=currentAssistant();if(message?.hotelFold==='folding')message.hotelFold='folded';}
function revealOne(){
 state.answerReveal?.control.resume();
...
function schedule(){stopPlayback();if(!state.paused||state.pendingLayer!==null){state.answerReveal?.control.resume();revealBeat();}}
function receive(e){
 state.queue.push(e);
...
function isBusy(){return Boolean(state.active||state.stopping||state.changingStock||currentAssistant()?.pending);}
function setBusy(busy){$('send').hidden=busy;$('stop').hidden=!busy||state.changingStock;$('composer').disabled=busy;$('stock-toggle').disabled=busy;$('semantic-toggle').disabled=busy;document.querySelectorAll('[data-prompt], [data-action="book-trip"], [data-action="retry"]').forEach(button=>button.disabled=busy);}
async function submit(text,options={}){
  ...
  clearReveal();stopPlayback();cancelFold();cancelAnswerReveal();const generation=++state.generation;state.queue=[];state.cursor=0;state.visible=[];state.pendingLayer=null;state.openLayer=null;state.follow=true;state.inspectedTurn=null;state.replaying=false;state.selectionTurn=null;
  state.paused=state.presenter&&state.semantic;state.tripExpanded=false;state.turn++;state.animatedEvents.clear();
  const user={role:'user',text},answer={role:'assistant',text:'',shownText:'',query:text,options:{...options},pending:true,turn:state.turn,bookingTurn:Boolean(options.confirmBooking),modelOnly:!state.semantic};state.messages.push(user,answer);state.chatAnchor='.user-message:nth-last-child(2)';renderTrip();
  const controller=new AbortController();
  $('composer').value='';$('chat-form').classList.remove('has-draft');state.active=controller;setBusy(true);renderChat();renderTraces();
  try{
    const response=await api('/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},signal:controller.signal,
    ...
    if(generation===state.generation&&!state.queue.some(e=>['done','error'].includes(e.type)))receive({type:'error',message:'The live stream ended before completion. Retry the question.'});
  }catch(error){if(generation===state.generation&&error.name!=='AbortError')receive({type:'error',message:error.message});}
  finally{if(generation===state.generation){state.active=null;renderChat();if(state.paused&&!state.visible.some(e=>e.type==='trace')&&!currentAssistant()?.failed){currentAssistant().status='The run has arrived. Choose Next layer or Resume to show it.';renderChat();}renderControls();if((!state.paused||state.pendingLayer!==null)&&!state.timer)schedule();}}
}
...
function openConnection(){
 ...
 const details=h?`<p class="dialog-lead">AWS account ${esc(h.accountId)} · ${esc(h.region)}</p><ul class="service-list">${h.services.map(s=>`<li><strong>${esc(s.name)}</strong><span class="${s.ok?'service-ok':'service-error'}">${esc(s.status)}</span>${s.details?`<small>${esc(JSON.stringify(s.details))}</small>`:''}</li>`).join('')}</ul><dl class="connection-details"><dt>Aurora</dt><dd>${esc(h.cluster)} / ${esc(h.database)}</dd><dt>Bedrock model</dt><dd>${esc(h.model)}</dd><dt>Runtime</dt><dd>${esc(h.runtimeId)}</dd><dt>Memory</dt><dd>${esc(h.memoryId)}</dd><dt>Gateway</dt><dd>${esc(h.gatewayId)}</dd><dt>Booking rules</dt><dd>${esc(h.policyEngineId)}</dd><dt>Journey graph</dt><dd>${esc(h.graphId)}</dd><dt>Source files</dt><dd>${esc(h.bucket)}</dd></dl><p>Checked ${esc(h.checkedAt)}. Each run shows its actual service requests and results.</p>`:`<p>${esc(state.healthError||'The connection check has not finished.')}</p><p>If your AWS session has expired, renew it using your usual profile, then check again.</p>`;
 openDialog('AWS connection and rehearsal',`${details}<button class="primary" data-action="refresh-connection">Check again ${icon('replay')}</button>${h?.ok?`<hr><h3>Prepare for another rehearsal</h3><p>Restore all fictional flight seats and hotel rooms to their starting counts. Saved booking records stay in the demo.</p><button class="quiet-button" data-action="reset-stock" ${isBusy()?'disabled':''}>Restore demo stock ${icon('replay')}</button>`:''}`);
}
...
async function refreshHealth(){
  ...
  try{const r=await api('/api/health');const h=await r.json();if(!r.ok)throw new Error(h.error);state.health=h;state.healthError=null;renderModels(h);$('connection-text').textContent=h.ok?'Online':'Needs attention';$('connection').classList.toggle('is-connected',h.ok);const stock=h.services.find(s=>s.name==='Aurora PostgreSQL')?.details?.ax218_seats;if(stock!==undefined)updateStock(stock);}
  catch(error){state.health=null;state.healthError=error.message;$('connection-text').textContent='Offline';$('connection').classList.remove('is-connected');announce(error.message);}
}
async function stopRequest(){
 const session=state.sessionId,wasLive=Boolean(state.active),message=currentAssistant();
 ++state.generation;state.active?.abort();state.active=null;stopPlayback();cancelFold();cancelAnswerReveal();state.selectionTurn=null;
 state.pendingLayer=null;state.paused=false;state.queue=state.queue.slice(0,state.cursor);
 if(!message)return;
 message.pending=false;message.savedEvents=[...state.visible];
 message.failed=message.booking?'Display stopped. The confirmed demo booking remains reserved.':message.bookingTurn?'Display stopped. A booking may already have completed. Retry this question to check the same booking safely.':'Display stopped. This run is incomplete.';
 state.stopping=wasLive;renderChat();renderControls();
 try{if(wasLive){const response=await api('/api/stop',{method:'POST',headers:{'Content-Type':'application/json'},signal:AbortSignal.timeout(12000),body:JSON.stringify({sessionId:session})});if(!response.ok)throw new Error('The runtime did not confirm the stop.');}}
 catch{message.failed+=' The server could not confirm that AWS execution stopped.';renderChat();}
 finally{state.stopping=false;renderControls();}
}
async function action(name,source){
  ...
  if(name==='presenter'||name==='auto-mode'){state.presenter=name==='presenter';state.paused=state.presenter;state.pendingLayer=null;stopPlayback();if(state.paused)state.answerReveal?.control.pause();else schedule();renderControls();announce(state.presenter?'Display paused. Advance an event or a layer.':'Events reveal at the selected pace.');}
  ...
  if(name==='play-pause'){state.paused=!state.paused;state.pendingLayer=null;if(state.paused){stopPlayback();state.answerReveal?.control.pause();}else schedule();renderControls();}
  ...
  if(name==='next-layer'){
    ...
    state.pendingLayer=next?next.layer:last?(last.phase==='result'?(currentAssistant()?.bookingTurn?SEMANTIC:Math.min(5,last.layer+1)):last.layer):0;
    revealBeat();
  }
  if(name==='replay'&&!isBusy()&&state.queue.some(e=>e.type==='done')){stopPlayback();cancelFold();cancelAnswerReveal();++state.generation;state.cursor=0;state.visible=[];state.animatedEvents.clear();state.follow=true;state.paused=true;state.replaying=true;state.inspectedTurn=null;state.pendingLayer=null;state.openLayer=null;Object.assign(currentAssistant(),{text:'',shownText:'',streamDone:false,plan:null,request:null,steps:[],hotelResults:null,hotelFold:null,travellerContext:null,booking:null,bookingRefused:null,policyDecision:null,currentLayer:undefined,pending:true,failed:null,replay:true,status:'Replay is paused. Choose Next layer or Resume.'});renderChat();renderTraces();}
  ...
  if(name==='retry'){const message=state.messages.find(m=>m.role==='assistant'&&m.turn===Number(source?.dataset.retryTurn))||currentAssistant();if(message?.query)await submit(message.query,message.options||{});}
  if(name==='book-trip'){if(isBusy()||state.bookingAttempted||state.selectionTurn===null||!state.semantic||state.replaying)return;await submit('Book this itinerary for Alex.',{confirmBooking:true});}
  if(name==='stop')await stopRequest();
  if(name==='new-chat'){if(state.stopping||state.changingStock)return;if(state.active||currentAssistant()?.pending)await stopRequest();clearReveal();stopPlayback();cancelFold();cancelAnswerReveal();++state.generation;state.sessionId='onward-'+crypto.randomUUID();state.messages=[];state.queue=[];state.visible=[];state.cursor=0;state.openLayer=null;state.pendingLayer=null;state.replaying=false;state.paused=state.presenter;state.cancelled=false;state.tripExpanded=true;state.inspectedTurn=null;state.selectionTurn=null;state.bookingAttempted=false;state.chatAnchor=null;renderTrip();renderChat();renderTraces();$('composer').value=initialQuery;showView(null);}
  ...
  if(name==='stock'||name==='reset-stock'){
    if(isBusy())return;
    state.changingStock=true;state.selectionTurn=null;renderControls();
    try{const r=await api('/api/scenario',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(name==='reset-stock'?{reset:true}:{soldOut:!state.soldOut})});const result=await r.json();if(!r.ok)throw new Error(result.error);state.soldOut=result.offer.seats===0;updateStock(result.offer.seats);announce(name==='reset-stock'?'The fictional seats and rooms have been restored for rehearsal.':`AX218 now has ${result.offer.seats} seats in Onward’s Aurora inventory.`);state.changingStock=false;if(name==='reset-stock'){$('detail-dialog').close();await refreshHealth();renderChat();}else if(state.messages.length&&state.semantic)await submit('Please recheck the current flight availability and find the best complete trip with my existing constraints.');}
    catch(error){openDialog('Inventory update',`<p>${esc(error.message)}</p>`);}
    finally{state.changingStock=false;renderControls();}
  }
...
document.addEventListener('click',event=>{const b=event.target.closest('button, a.wordmark, a[data-action]');if(!b||b.disabled)return;
  if(b.matches('a.wordmark')){event.preventDefault();showView(null);return;}
...
window.addEventListener('pagehide',()=>{clearReveal();stopPlayback();cancelFold();++state.generation;state.active?.abort();});
...
function updateStock(seats){state.soldOut=seats===0;$('stock-label').textContent=seats===0?'Sold out':`${seats} ${seats===1?'seat':'seats'}`;$('stock-toggle').setAttribute('aria-label',state.soldOut?'Restore flight availability in Aurora':'Sell out the preferred flight in Aurora');}
