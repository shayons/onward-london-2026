...
function bookRow(turn){
 ...
 return `<div class="book-row"><button class="primary book-button" data-action="book-trip" ${state.active?'disabled':''}>${icon('check')} Book this for Alex</button><span>One click confirms. The gateway’s Cedar policy decides before the booking tool runs; the inventory is fictional.</span></div>`;
}
...
function apply(e){
  ...
  if(e.type==='booking'){message.booking=e.booking||null;message.bookingRefused=e.booking?null:(e.refused||'The gateway did not complete the booking.');message.policyDecision=e.policyDecision||null;state.chatAnchor='.assistant-message:last-child .booking-card, .assistant-message:last-child .booking-refused';}
  if(e.type==='trace'){
...
async function submit(text,options={}){
  ...
  state.paused=state.presenter;state.tripExpanded=false;state.lastQuery=text;state.lastOptions=options;state.turn++;state.animatedEvents.clear();
  ...
  try{
    const response=await api('/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},signal:state.active.signal,
      body:JSON.stringify({message:text,sessionId:state.sessionId,...(options.confirmBooking?{confirmBooking:true}:{}),...(state.memoryOverride!==null?{memoryEnabled:state.memoryOverride}:{}),...(state.modelId?{modelId:state.modelId}:{})})});
