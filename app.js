...
import {hotelImages,hotelTiles,serviceBadges,serviceNames,layerServices,layerContributions,preparation,briefing,talkNotes} from './presentation.js?v=briefing-trace-detail-20260909';
...
const state={sessionId:'onward-'+crypto.randomUUID(),messages:[],health:null,active:null,cancelled:false,presenter:false,modelId:null,
  ...
  soldOut:false,memory:true,memoryOverride:null,inspectedTurn:null,replaying:false,queue:[],cursor:0,visible:[],pendingLayer:null,lastQuery:'',turn:0,tripExpanded:true,animatedEvents:new Set(),preparing:false,dialogFocus:null};
...
function sources(turn){return `<div class="answer-sources" aria-label="Answer sources">${layers.map((l,i)=>`<button data-source="${i}" data-source-turn="${turn}">[${i+1}] ${l.name}</button>`).join('')}</div>`;}
...
function reasoning(m){
 ...
 const rows=steps.map(step=>`<li class="step ${step.done?'done':'active'}"><span class="step-mark">${step.done?icon('check'):'<span class="step-pulse"></span>'}</span><div><strong>${esc(layers[step.layer].name)}</strong><p>${esc(step.summary||'')}</p><span class="step-marks">${serviceMarks(layerServices[step.layer])}</span></div></li>`).join('');
 if(!opening&&!rows)return '';
 const body=`<ol class="reasoning">${opening}${rows}</ol>`;
 if(m.pending)return body;
 const done=steps.filter(step=>step.done).length;
 return `<details class="reasoning-done"><summary>${done} of 6 semantic responsibilities resolved ${icon('chevron')}</summary>${body}</details>`;
}
...
function renderControls(){
 const pending=state.queue.length-state.cursor,traces=traceEvents().filter(e=>e.type==='trace'),complete=new Set(traces.filter(e=>e.phase==='result').map(e=>e.layer)).size;
 ...
 $('trace-progress').innerHTML=`<span>${traces.length?`${complete} / 6 components · ${traces.length} semantic events`:'Prepared data becomes a supported answer.'}</span>${pending?`<small>${pending} stream events buffered</small>`:''}`;
...
function renderTraces(){
 ...
 $('layers').innerHTML=layers.map((l,i)=>{
  const ev=shown.filter(e=>e.type==='trace'&&e.layer===i),last=ev.at(-1),done=ev.some(e=>e.phase==='result'),open=state.openLayer===i,error=shown.findLast(e=>e.type==='error'&&e.layer===i);
  const resolution=layerResolution(i,ev);
  return `<li class="layer ${last?'has-events':''} ${done?'complete':''} ${open?'expanded':''}"><button class="layer-toggle" data-layer="${i}" aria-expanded="${open}" aria-controls="layer-body-${i}"><span class="layer-number">${done?icon('check'):i+1}</span><span class="layer-title"><strong>${l.name}</strong><span>${l.service}</span></span><span class="layer-state">${error?'Error':done?'Resolved':last?'Active':''}${icon('chevron')}</span></button><div id="layer-body-${i}" class="layer-body" ${open?'':'hidden'}><h3 class="layer-question">${l.question}</h3><div class="layer-resolution"><div><span class="layer-resolution-label">Question phrase</span><strong>${esc(l.phrase)}</strong></div><div><span class="layer-resolution-label">Resolved meaning</span><strong>${esc(resolution.value)}</strong></div><p>${esc(resolution.detail)}</p></div><p class="layer-contribution">${layerContributions[i]}</p>${serviceBadges(layerServices[i],i)}${ev.length?ev.map(e=>{
...
function setBusy(busy){$('send').hidden=busy;$('stop').hidden=!busy;$('composer').disabled=busy;$('stock-toggle').disabled=busy;$('memory-toggle').disabled=busy;}
async function submit(text){
  ...
  state.paused=state.presenter;state.tripExpanded=false;state.lastQuery=text;state.turn++;state.animatedEvents.clear();
  const user={role:'user',text},answer={role:'assistant',text:'',pending:true,turn:state.turn};state.messages.push(user,answer);state.chatAnchor='.user-message:nth-last-child(2)';renderTrip();
  ...
  try{
    const response=await api('/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},signal:state.active.signal,
      body:JSON.stringify({message:text,sessionId:state.sessionId,...(state.memoryOverride!==null?{memoryEnabled:state.memoryOverride}:{}),...(state.modelId?{modelId:state.modelId}:{})})});
  ...
  finally{state.memoryOverride=null;state.active=null;setBusy(false);if(state.paused&&!state.visible.some(e=>e.type==='trace')&&!currentAssistant()?.failed){currentAssistant().status='The live run is ready. Advance an event or a layer to reveal its evidence.';renderChat();}renderControls();if((!state.paused||state.pendingLayer!==null)&&!state.timer)schedule();}
}
function announce(message){$('announcement').textContent=message;}
function updateMemory(){const b=$('memory-toggle');b.setAttribute('aria-pressed',String(state.memory));b.setAttribute('aria-label',`AgentCore Memory ${state.memory?'on':'off'}`);b.innerHTML=`<span>AgentCore Memory</span><span class="switch-track"></span><span class="sr-only">${state.memory?'on':'off'}</span>`;}
...
async function action(name,source){
  ...
  if(name==='replay'&&!state.active){stopPlayback();state.cursor=0;state.visible=[];state.animatedEvents.clear();state.follow=true;state.paused=true;state.replaying=true;state.inspectedTurn=null;state.pendingLayer=null;state.openLayer=null;Object.assign(currentAssistant(),{text:'',plan:null,hotelResults:null,currentLayer:undefined,pending:true,failed:null,replay:true,status:'Replay is paused. Advance an event or a semantic layer.'});renderChat();renderTraces();}
  if(name==='memory'){const text=state.memory?'Ignore remembered preferences and choose a hotel on the current trip constraints.':'Restore my remembered hotel preferences.';if(state.messages.length)await submit(text);else{state.memory=!state.memory;state.memoryOverride=state.memory;updateMemory();announce(state.memory?'Memory will be used for this question.':'Memory will be disabled for this question.');}}
  if(name==='retry')await submit(state.lastQuery);
  if(name==='stop')await stopRequest();
  if(name==='new-chat'){if(state.active)await stopRequest();clearReveal();stopPlayback();state.sessionId='onward-'+crypto.randomUUID();state.messages=[];state.queue=[];state.visible=[];state.cursor=0;state.openLayer=null;state.pendingLayer=null;state.replaying=false;state.paused=state.presenter;state.cancelled=false;state.tripExpanded=true;state.memory=true;state.memoryOverride=null;state.inspectedTurn=null;updateMemory();renderTrip();renderChat();renderTraces();$('composer').value=initialQuery;$('composer').focus();}
...
renderTrip();renderChat();renderTraces();updateMemory();$('composer').value=initialQuery;refreshHealth();showView(viewForPath(location.pathname),false);
